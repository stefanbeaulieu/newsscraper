$(document).ready(function () {
	...
	function renderNotesList(data) {
		...
		if (!data.notes.length) {
			...
			for (var i; i < data.notes.length; i++ ) {
				...
				// Store the note id onthe delete button for east access hen trying to delete
				currentNote.children("button").data("_id", data.notes[i]._id);
				// Adding our currentNote to be notesToRender array
				notesToRender.push(currentNote);
			}
		}
		// Now append the notesToRender array
	...
	function handleArticleNotes() {
		...
		var current Article = $(this).parents(".panel").data();
		...
		$.get("/api/notes/" + currentArticle._id).then(function (data) {
			...
			// Adding the formatted HTML o the note modal
			bootbox.dialog({
				message: modalText,
				closeButton: true
			});
